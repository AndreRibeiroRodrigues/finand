(() => {
  ...
  const elements = { modal: $('#entryModal'), form: $('#entryForm'), body: $('#entriesBody'), empty: $('#emptyState'), search: $('#searchInput'), count: $('#entryCount'), total: $('#grandTotal'), result: $('#resultLabel'), title: $('#modalTitle'), toast: $('#toast') };
  let entries = loadEntries();
  let lastFocusedElement = null;

  function loadEntries() { try { const value = JSON.parse(localStorage.getItem(STORAGE_KEY)); return Array.isArray(value) ? value : []; } catch { return []; } }
  ...
  function render() {
    ...
    const filtered = entries.filter((entry) => Object.values(entry).some((value) => String(value).toLocaleLowerCase('pt-BR').includes(query)));
    elements.body.innerHTML = filtered.map((entry) => {
      const date = new Date(`${entry.date}T12:00:00`);
      return `<tr>
        <td>${formatDate(entry.date)}</td><td>${monthNames[date.getMonth()]}</td><td>${date.getFullYear()}</td>
        <td>${escapeHtml(entry.category)}</td><td>${escapeHtml(entry.subcategory || '—')}</td><td>${escapeHtml(entry.description)}</td>
        <td class="amount">${formatCurrency(entry.amount)}</td><td><span class="badge ${entry.paid ? 'yes' : 'no'}">${entry.paid ? 'Sim' : 'Não'}</span></td>
        <td>${escapeHtml(entry.paymentMethod)}</td><td>${escapeHtml(entry.notes || '—')}</td>
  ...
  function closeModal() { elements.modal.hidden = true; document.body.style.overflow = ''; lastFocusedElement?.focus(); }

  elements.form.addEventListener('submit', (event) => {
    event.preventDefault();
    const id = $('#entryId').value;
    const entry = { id: id || (crypto.randomUUID?.() || `${Date.now()}-${Math.random()}`), date: $('#date').value, category: $('#category').value.trim(), subcategory: $('#subcategory').value.trim(), description: $('#description').value.trim(), amount: Number($('#amount').value), paid: $('#paid').value === 'true', paymentMethod: $('#paymentMethod').value, notes: $('#notes').value.trim() };
    if (id) entries = entries.map((item) => item.id === id ? entry : item); else entries.unshift(entry);
    saveEntries(); render(); closeModal(); showToast(id ? 'Lançamento atualizado.' : 'Lançamento adicionado.');
  });
  elements.body.addEventListener('click', (event) => {
    ...
    if (button.dataset.action === 'delete' && confirm(`Excluir “${entry.description}”?`)) { entries = entries.filter((item) => item.id !== entry.id); saveEntries(); render(); showToast('Lançamento excluído.'); }
  });
  $('#newEntryButton').addEventListener('click', () => openModal());
  $('#closeModalButton').addEventListener('click', closeModal); $('#cancelButton').addEventListener('click', closeModal);
